...
function calculateScore(query, content, title) {
...
function extractSnippet(content, query, contextLength = 200) {
    const lowerContent = content.toLowerCase();
    const lowerQuery = query.toLowerCase();
    const queryTokens = tokenize(query);
...
export function searchDocs(index, query, limit = 10) {
    ...
    for (const page of index.pages) {
        ...
        if (score > 0) {
            ...
            const matchedSections = allSections.filter(section => calculateScore(query, section.content, section.title) > 0);
            results.push({
                score,
                page,
...
export function searchByCategory(index, category) {
    return index.pages.filter(page => page.category.toLowerCase() === category.toLowerCase());
}
export function getPageByPath(index, path) {
    // Normalize path
    const normalizedPath = path.startsWith('/') ? path : `/${path}`;
    return index.pages.find(page => page.path === normalizedPath ||
...
export function listAllPages(index) {
