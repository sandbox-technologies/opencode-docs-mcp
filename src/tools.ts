...
export function registerDocTools(server: McpServer, getIndex: () => DocsIndex | null): void {
  ...
  // Search docs tool
  server.tool(
    'search_opencode_docs',
    'Search the OpenCode documentation for relevant information. Use this to find docs about configuration, usage, MCP servers, agents, and more.',
    {
...
${sections ? `**Relevant sections:**\n${sections}` : ''}`;
  ...
  // Get specific page
  server.tool(
    'get_opencode_doc_page',
    'Get the full content of a specific OpenCode documentation page by its path.',
    {
  ...
  server.tool(
    'list_opencode_docs_by_category',
    'List all OpenCode documentation pages in a specific category.',
    {
  ...
  // List all categories
  server.tool(
    'list_opencode_doc_categories',
    'List all available categories in the OpenCode documentation.',
    {},
    async () => {
      const index = getIndex();
      if (!index) {
  ...
  // Browse all docs
  server.tool(
    'browse_opencode_docs',
    'Get an overview of the entire OpenCode documentation structure.',
    {},
    async () => {
      const index = getIndex();
      if (!index) {
